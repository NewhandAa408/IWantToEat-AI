...
if st.session_state.search_clicked:
    ...
    # 3. 高度擬真的台中市餐廳資料庫 (加入真實評分與網友評論)
    with st.spinner("正在載入精選餐廳資料庫..."):
        demo_database = [
            {"name": "屋馬燒肉 (國安店)", "cuisine": "日式燒肉", "price": 1500, "rating": 4.8, "lat": 24.192, "lon": 120.603, "options": ["不吃海鮮", "不吃香菜", "不吃辣"], "reviews": "雞湯超好喝可以無限續，海鮮粥必點，肉質鮮嫩服務極佳！"},
            {"name": "輕井澤鍋物 (文心南二店)", "cuisine": "日式火鍋", "price": 400, "rating": 4.5, "lat": 24.131, "lon": 120.647, "options": ["不吃辣", "不吃海鮮", "不吃香菜", "無麩質"], "reviews": "裝潢大氣，壽喜燒湯頭偏甜很下飯，紅茶好喝，CP值極高。"},
            {"name": "赤鬼炙燒牛排 (台灣大道店)", "cuisine": "台式牛排", "price": 380, "rating": 4.2, "lat": 24.156, "lon": 120.654, "options": ["不吃海鮮", "不吃香菜"], "reviews": "酥脆麵包跟羅宋湯很讚，牛排份量大，黑胡椒醬夠味。"},
            {"name": "宮原眼科", "cuisine": "冰品甜點", "price": 250, "rating": 4.4, "lat": 24.137, "lon": 120.683, "options": ["素食", "不吃海鮮", "不吃辣", "不吃香菜", "無麩質"], "reviews": "冰淇淋口味超多，巧克力系列必吃，還可以免費加鳳梨酥跟乳酪蛋糕！"},
            {"name": "沁園春", "cuisine": "江浙菜/小籠包", "price": 800, "rating": 4.3, "lat": 24.138, "lon": 120.683, "options": ["不吃辣", "不吃海鮮"], "reviews": "小籠包皮薄汁多，排骨炒飯粒粒分明，老字號招牌玫瑰包必點。"},
            {"name": "刁民酸菜魚 (逢甲店)", "cuisine": "川菜/酸菜魚", "price": 600, "rating": 4.6, "lat": 24.176, "lon": 120.645, "options": ["不吃海鮮", "不吃香菜"], "reviews": "湯頭酸爽麻辣超級下飯！洛神花茶解辣神物，裡面加點老油條超好吃。"},
            {"name": "飪室咖哩 RenshiCurry", "cuisine": "印度咖哩", "price": 300, "rating": 4.5, "lat": 24.148, "lon": 120.655, "options": ["不吃海鮮", "無麩質"], "reviews": "初戀咖哩帶點番茄酸甜，起司烤餅拉絲超浮誇，裝潢很網美。"},
            {"name": "老向的店", "cuisine": "北方麵食", "price": 120, "rating": 4.4, "lat": 24.172, "lon": 120.680, "options": ["不吃海鮮", "不吃辣"], "reviews": "清蒸鴨腿麵晚來吃不到！蒜泥白肉份量多，平價吃粗飽的神店。"},
            {"name": "茶六燒肉堂 (朝富店)", "cuisine": "日式燒肉", "price": 1600, "rating": 4.7, "lat": 24.164, "lon": 120.636, "options": ["不吃海鮮", "不吃香菜", "不吃辣"], "reviews": "鮭魚味噌湯料超多，鹽蔥牛舌絕配，最後的雪花冰是完美結尾。"},
            {"name": "斐得蔬食", "cuisine": "創意素食", "price": 450, "rating": 4.8, "lat": 24.140, "lon": 120.665, "options": ["素食", "不吃海鮮", "不吃辣", "不吃香菜"], "reviews": "完全吃不出是素食！松露燉飯香氣濃郁，植物肉漢堡排口感驚豔。"}
        ]
    ...
    if len(filtered_restaurants) > 0:
        ...
        with st.container(border=True):
            ...
            with col1:
                ...
                st.info(f"💬 **網友評價**：{top1['reviews']}") # 用 st.info 把評價框起來凸顯

            with col2:
                st.write("#### ✨ AI 特選專屬菜單")
                
                # 【關鍵】：如果記憶體裡面還沒有菜單，才呼叫 AI
                ...
                # 如果記憶體裡面有菜單，就直接印出來 (這樣地圖刷新時就不會重跑 AI)
        ...
        display_data = [
                {
                    ...
                    "餐廳名稱": r["name"], 
                    "風格": r["cuisine"],
                    "推薦分數": f"{r['推薦分數']} 分",  # 演算法分數
                    "真實評分": f"⭐️ {r['rating']}",   # 真實星級
                    "網友評價": r["reviews"],          # 加上這行！
                    "價格": f"${r['price']}", 
                    "距離(m)": r["distance"]
                } 
